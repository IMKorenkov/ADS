...
mounted() {
    setInterval(() => {
        let EPSILON = 0.001,
            ...
            l = 1;
    this.p = this.activSystems.getNextPoint(this.p);
    this.pc = this.activSystems.getNextPoint(this.pc);
    this.pc.x -= this.p.x; this.pc.y -= this.p.y; this.pc.z -= this.p.z;
    l = this.pc.length();
    this.s += Math.log(l/EPSILON);
    this.pc.x = this.p.x + this.pc.x/l*EPSILON;
    this.pc.y = this.p.y + this.pc.y/l*EPSILON;
    this.pc.z = this.p.z + this.pc.z/l*EPSILON;
    this.countPoints++;
    this.result = this.s/(this.countPoints*EPSILON)
    });
},
watch: {
